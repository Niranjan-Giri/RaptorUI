...
export function createUIManager(app, sceneManager, queryHandler) {
    ...
    function showInlineQueryMessage(text, type = 'info', duration = 3000) {
    ...
    function exportSceneInfo() {
        ...
        const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'scene-info.generated.json'; a.style.display = 'none'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
    }
    ...
    function setMode(mode) {
        ...
        document.querySelectorAll('.control-btn, .icon-btn').forEach(btn => btn.classList.remove('active'));
        if (mode === 'orbit') {
    ...
    function ensureSceneInfoForFile(filename) {
        if (!app.sceneInfo) app.sceneInfo = { name: {}, bounding_box: {}, labels: {}, _map: new Map(), displayNames: new Map() };
        ...
        if (!existingEntryKey) {
            const basename = filename.replace(/\.ply$/i, ''); let key = basename; let suffix = 1; while (app.sceneInfo.name && app.sceneInfo.name[key]) key = `${basename}_${suffix++}`;
            app.sceneInfo.name = app.sceneInfo.name || {}; app.sceneInfo.name[key] = filename;
        }
        const geometry = fileData.geometry; geometry.computeBoundingBox(); const bbox = geometry.boundingBox; const size = new THREE.Vector3(); bbox.getSize(size);
        const centerLocal = new THREE.Vector3(); bbox.getCenter(centerLocal);
        let centerWorld = centerLocal.toArray(); if (fileData.object) { const cw = centerLocal.clone(); fileData.object.localToWorld(cw); centerWorld = cw.toArray(); }
        ...
        if (!app.sceneInfo.bounding_box[keyForFile]) app.sceneInfo.bounding_box[keyForFile] = { x: size.x, y: size.y, z: size.z, center: centerWorld };
        app.sceneInfo.labels = app.sceneInfo.labels || {}; app.sceneInfo.labels[filename] = app.sceneInfo.labels[filename] || [filename.replace(/\.ply$/i, '')];
        app.sceneInfo._map = app.sceneInfo._map || new Map(); const basenameLower = filename.replace(/\.ply$/i, '').toLowerCase(); app.sceneInfo._map.set(basenameLower, { key: Object.keys(app.sceneInfo.name).find(k => app.sceneInfo.name[k] === filename), filename });
        if (app.sceneInfo.labels[filename]) { for (const lab of app.sceneInfo.labels[filename]) { const labLower = String(lab).toLowerCase(); app.sceneInfo._map.set(labLower, { key: Object.keys(app.sceneInfo.name).find(k => app.sceneInfo.name[k] === filename), filename }); } app.sceneInfo.displayNames = app.sceneInfo.displayNames || new Map(); app.sceneInfo.displayNames.set(filename, app.sceneInfo.labels[filename]); }
    }
